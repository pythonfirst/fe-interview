...
```js
/**
 * 截流：某一个事件段内只能执行1次，比如 wait=1000ms 时间内只能执行1次。
 * 1. 返回一个函数
 * 2. 保留原函数 this 环境
 * 3. 支持原函数参数
 * @param {*} fn 
...
function debounce(fn, wait) {
  ...
  return function() {
    ...
    let now = + new Date();
    console.log(now, prev,now - prev, wait)
    if (now - prev > wait) {
      prev = + new Date();
      fn.apply(self, args);
    }
...
1. 创建一个新对象
2. 新对象的原型（__proto__) 指向构造函数的 原型对象（这样新对象就可以继承构造函数原型上的属性和方法了）
3. 通过 新对象 执行构造函数，让新对象能够访问构造函数的属性；
4. 返回新对象。
...
const child = _new(Parent, 'zhao', 30)
...
function _new() {
  ...
  const F = [].shift.call(arguments);
  obj.__proto__ = F.prototype;
  const res = F.apply(obj, arguments)
  // 如果构造函数返回一个对象则返回res，否则返回新对象
...
function cb2() {
  console.log('我也调用了 sayName');
}

bus.once('sayName', cb1)
bus.once('sayName', cb2)
console.log('bus', bus)
...
借助辅助函数 traverse，去递归调用，关键理解 parentid（pid)与id之间的关系。
...
/**
 * 函数接收一个列表，和当前层级，返回一个当前层的树；
 * @param {*} list 列表
 * @param {*} pid 当前层数
 */
function listToTree(list) {

  function traverse(list, pid=null) {
    let res = [];
    for (let item of list) {
      if (item.pid === pid) {
        // 这句最重要
        const children = traverse(list, item.id);
        if (children.length) {
...
/**
 * 1. 用 map 记录id 与 item
 * 2. 遍历 item 找 parent，并添加到 parent.children
 * @param {*} list 列表
 * @param {*} pid 当前层数
 */
function listToTree(list) {
